    ...
    <div class="row mb-2">
        <div class="col-8 search">
            <form method="GET" action="{{ \App\Helpers\Params::buildUrl(false, 'search-key') }}">
                ...
                <input type="text" class="search-input" name="search-key" placeholder="名前などを入力"
                    value="{{ $searchKey }}">
            </form>
        </div>
    </div>
        ...
        <div class="col-md-8">
            <div class="block-filter">
                <div class="contain-filter">
                    <input id="datepicker-birthday" class="js-datepicker" type="text"
                        style="width: 0; padding: 0; opacity: 0; position: relative; left: 30px; top: 5px"
                        data-url="{{ \App\Helpers\Params::buildUrl('birthdayValue', 'birthday') }}"
                    ...
                    <select name="area" id="area" class="js-href-value">
                        ...
                        @foreach($area as $ar)
                        <option {{ \App\Helpers\Params::setSelected('area', $ar->id) }}
                    ...
                    <select name="careersList-filter" id="careersList">
                        ...
                        <option {{ \App\Helpers\Params::setSelected('career_id', $career->id) }}
                            value="{{ \App\Helpers\Params::buildUrl($career->id, 'career_id') }}">{{ $career->title }}
                        </option>
                        @endforeach
                    </select>
                </div>
    ...
    <div class="row block-content">
        <div class="col-md-8">
            ...
            <div class="item">
                <div class="head">
                    <div class="avatar">
                        <img src="{{ \App\Helpers\ImageRender::userAvatar($user->avatar) }}" alt="avatar"
                            class="rounded-circle" width="47" height="47" style="border-radius: 50%">
                    </div>
                    <div class="action">
                        ...
                        <div class="action-social">
                            ...
                            <div class="action-social_list">
                                <div class="action-social_list-item"><img
                                        src="{{asset('/assets/images/icon-sm/tw.svg')}}" alt="twitter">
                                    {{ \App\Helpers\Format::numberFormat(isset($snsFollowersCount[$user->id]['twitter'])  ?: 0, true) }}
                                </div>
                                <div class="action-social_list-item"><img
                                        src="{{asset('/assets/images/icon-sm/insta.svg')}}" alt="insta">
                                    {{ \App\Helpers\Format::numberFormat(isset($snsFollowersCount[$user->id]['instagram']) ?: 0, true) }}
                                </div>
                                <div class="action-social_list-item"><img
                                        src="{{asset('/assets/images/icon-sm/youtube.svg')}}" alt="youtube">
                                    {{ \App\Helpers\Format::numberFormat(isset($snsFollowersCount[$user->id]['youtube']) ?: 0, true) }}
                                </div>
                                <div class="action-social_list-item"><img
                                        src="{{asset('/assets/images/icon-sm/tiktok.svg')}}" alt="tiktok">
                                    {{ \App\Helpers\Format::numberFormat(isset($snsFollowersCount[$user->id]['tiktok']) ?: 0, true) }}
                                </div>
                            </div>
                        </div>
                        <div class="action-portfolio">
                            ...
                            <img src="{{ \App\Helpers\ImageRender::portfolioAvatar($img) }}" alt=""
                                style="border-radius: 4px" width="60" height="47">
                            @endforeach
                        </div>
                    </div>
                    <div class="info">
                    ...
                    <div class="account">
                        ...
                        <div class="account-created">
                            ...
                            <span
                                class="content-item">{{ \App\Helpers\DateTime::showDateTime($user->created_at) }}</span>
                        </div>
                        <div class="account-updated">
                            <span class="title-item">更新日時</span>
                            <span
                                class="content-item">{{ \App\Helpers\DateTime::showDateTime($user->updated_at) }}</span>
                        </div>
                    </div>
                </div>
                <div class="hr"></div>
                <div class="footer">
                    <a href="{{ \App\Helpers\Helper::concatStringBySlash(config('common.frontend_url'), $user->user_name) }}"
                        class="item-button detail">詳細</a>
                    <button class="item-button email sendEmail" data-user-id="{{ $user->id }}"
                        data-user-name="{{ $user->given_name ?: $user->username }}">メール</button>
                    <div class="contain-filter">
                        <select name="change-status" id="change-status"
                            data-url="{{ route('admin.user.update.status', ['id' => $user->id]) }}">
                            <option value="{{ \App\Entities\User::STATUS_ACTIVE }}" @if ($user->active ==
                                \App\Entities\User::STATUS_ACTIVE) selected @endif>公開</option>
                            <option value="{{ \App\Entities\User::STATUS_INACTIVE }}" @if ($user->active ==
                                \App\Entities\User::STATUS_INACTIVE) selected @endif>非公開</option>
                        </select>
                    </div>
                    <a href="{{ route('admin.users.delete', ['id' => $user->id]) }}"
                        data-redirect="{{ \App\Helpers\Params::buildUrl(false, 'page') }}" data-method="DELETE"
