...
var Admin = (function () {
    return {
        ...
        doSendEmailToUser: function (form) {
            ...
            $.post({
                ...
                if (data.hasOwnProperty("success") && data.success) {
                    ...
                    form.find(".ajax-response.text-success")
                        .html(data.message)
                        .show();
                }
                if (data.hasOwnProperty("success") && data.success == false) {
                    form.find(".ajax-response.text-danger")
                        .html(data.message)
                        .show();
                }
        ...
        userPortfolioPage: function () {
            ...
            $(".change-active-portfolio").on("change", function (e) {
                var $this = $(this);
                var val = $(this).val();
                $.ajax({
                    type: $this.data("method"),
                    data: { statusActive: val },
                    url: $this.data("url-change-status"),
                }).done(function (data) {
                    if (data.hasOwnProperty("success") && data.success) {
                        window.location.reload();
